...
valueSmooth=values.copy()

n=5
for i in range(n, len(data) - n):
    valueSmooth[i]=1.0/11.0*(values[i-5]+values[i-4]+values[i-3]+values[i-2]+values[i-1]+values[i]+values[i+1]+values[i+2]+values[i+3]+values[i+4]+values[i+5])
...
plt.plot(timestamps, valueSmooth, linestyle='-',color='r')
